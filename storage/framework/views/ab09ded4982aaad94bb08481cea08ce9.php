<?php $__env->startSection('title', 'Commande #' . ($order->order_number ?? '') . ' - EpiDrive'); ?>
<?php $__env->startSection('meta_description', 'Details de votre commande EpiDrive.'); ?>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 relative z-10">
            <nav class="text-sm text-emerald-200 mb-4">
                <ol class="flex items-center space-x-2 flex-wrap">
                    <li><a href="<?php echo e(route('home')); ?>" class="hover:text-white transition">Accueil</a></li>
                    <li><span class="text-emerald-300/50">/</span></li>
                    <li><a href="<?php echo e(route('account.index')); ?>" class="hover:text-white transition">Mon compte</a></li>
                    <li><span class="text-emerald-300/50">/</span></li>
                    <li><a href="<?php echo e(route('account.orders')); ?>" class="hover:text-white transition">Mes commandes</a></li>
                    <li><span class="text-emerald-300/50">/</span></li>
                    <li class="text-white font-medium">#<?php echo e($order->order_number ?? ''); ?></li>
                </ol>
            </nav>
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-white">Commande #<?php echo e($order->order_number ?? ''); ?></h1>
                    <p class="text-emerald-100 mt-1">Passee le <?php echo e($order->created_at->format('d/m/Y a H:i') ?? ''); ?></p>
                </div>
                    ...
                    <div class="flex items-center justify-between relative">
                        ...
                        <?php if($currentStepIndex >= 0): ?>
                            <div class="absolute top-5 left-0 h-1 bg-gradient-to-r from-emerald-600 to-emerald-500 rounded-full mx-8 transition-all duration-1000"
                                 style="width: calc(<?php echo e(($currentStepIndex / (count($steps) - 1)) * 100); ?>% - 4rem);"></div>
                        <?php endif; ?>
                        ...
                        <?php $__currentLoopData = $steps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $step): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="relative z-10 flex flex-col items-center" style="width: 20%;">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-500
                                    <?php echo e($i <= $currentStepIndex ? 'bg-gradient-to-br from-emerald-600 to-emerald-500 shadow-lg shadow-emerald-500/30' : 'bg-gray-200'); ?>">
                                    <svg class="w-5 h-5 <?php echo e($i <= $currentStepIndex ? 'text-white' : 'text-gray-400'); ?>" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <?php echo $stepIcons[$step]; ?>

                                    </svg>
                                </div>
                                <span class="mt-2 text-xs font-bold <?php echo e($i <= $currentStepIndex ? 'text-emerald-700' : 'text-gray-400'); ?>">
                                    <?php echo e($stepLabels[$step]); ?>
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="relative">
                                <div class="w-8 h-8 rounded-full flex items-center justify-center
                                    <?php echo e($i <= $currentStepIndex ? 'bg-gradient-to-br from-emerald-600 to-emerald-500' : 'bg-gray-200'); ?>">
                                    <svg class="w-4 h-4 <?php echo e($i <= $currentStepIndex ? 'text-white' : 'text-gray-400'); ?>" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <?php echo $stepIcons[$step]; ?>
                                ...
                                <?php if(!$loop->last): ?>
                                    <div class="absolute top-8 left-1/2 -translate-x-1/2 w-0.5 h-3
                                        <?php echo e($i < $currentStepIndex ? 'bg-emerald-500' : 'bg-gray-200'); ?>"></div>
                                <?php endif; ?>
                            </div>
                            <span class="text-sm font-bold <?php echo e($i <= $currentStepIndex ? 'text-emerald-700' : 'text-gray-400'); ?>">
                                <?php echo e($stepLabels[$step]); ?>

                            </span>
                            <?php if($i == $currentStepIndex): ?>
            ...
            <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 overflow-hidden">
                        ...
                        <div class="flex items-center p-4 sm:p-6 space-x-4 hover:bg-gray-50/50 transition">
                            <div class="flex-shrink-0">
                                <?php if($item->product && $item->product->image): ?>
                                    <img src="<?php echo e(asset('storage/' . $item->product->image)); ?>" alt="<?php echo e($item->product_name); ?>"
                            ...
                            <div class="flex-1 min-w-0">
                                <h3 class="font-bold text-gray-800 text-sm sm:text-base"><?php echo e($item->product_name); ?></h3>
                                <p class="text-sm text-gray-400 mt-1">
                                    <?php echo e(number_format($item->unit_price, 2, ',', ' ')); ?> &euro; x <?php echo e($item->quantity); ?>

                                </p>
                            </div>
                            <div class="text-right">
                                <p class="font-extrabold text-gray-900"><?php echo e(number_format($item->total_price, 2, ',', ' ')); ?> &euro;</p>
                            </div>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
                ...
                <div class="bg-gradient-to-r from-gray-50 to-gray-50/50 p-6 space-y-3 text-sm">
                    <div class="flex justify-between text-gray-600">
                        ...
                        <span class="font-semibold"><?php echo e(number_format($order->subtotal ?? 0, 2, ',', ' ')); ?> &euro;</span>
                    </div>
                    <div class="flex justify-between text-gray-600">
                        <span>Frais de livraison</span>
                        <span class="font-semibold"><?php echo e(number_format($order->delivery_fee ?? 0, 2, ',', ' ')); ?> &euro;</span>
                    </div>
                    <div class="flex justify-between text-lg font-extrabold text-gray-900 pt-3 border-t border-gray-200">
                        <span>Total</span>
                        <span class="text-emerald-600"><?php echo e(number_format($order->total ?? 0, 2, ',', ' ')); ?> &euro;</span>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 p-6">
                    ...
                    <div class="space-y-4 text-sm">
                        <div class="bg-gray-50 rounded-xl p-4">
                            ...
                            <p class="text-gray-700 font-medium"><?php echo e($order->customer_address ?? 'Non renseignee'); ?></p>
                        </div>
                        <?php if($order->delivery_instructions ?? false): ?>
                            <div class="bg-gray-50 rounded-xl p-4">
                                ...
                                <p class="text-gray-700 font-medium"><?php echo e($order->delivery_instructions); ?></p>
                            </div>
                        <?php endif; ?>
                        <div class="bg-gray-50 rounded-xl p-4">
                            <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Creneau</p>
                            <p class="text-gray-700 font-medium"><?php echo e($order->delivery_slot ?? 'Non defini'); ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 p-6">
                    ...
                    <div class="space-y-4 text-sm">
                        <div class="bg-gray-50 rounded-xl p-4">
                            ...
                            ?>
                            <p class="text-gray-700 font-medium"><?php echo e($paymentLabels[$order->payment_method] ?? $order->payment_method ?? 'Non defini'); ?></p>
                        </div>
                        <div class="bg-gray-50 rounded-xl p-4">
                            ...
                            <?php else: ?>
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-700 ring-1 ring-gray-200">
                                    <?php echo e($order->payment_status ?? 'Inconnu'); ?>
                        ...
                        <div class="bg-gray-50 rounded-xl p-4">
                            ...
                            <p class="text-gray-700 font-medium"><?php echo e($order->customer_phone ?? Auth::user()->phone ?? 'Non renseigne'); ?></p>
                        </div>
                    </div>
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/account/order-detail.blade.php ENDPATH**/ ?>