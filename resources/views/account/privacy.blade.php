...
@section('title', 'Mes donnees personnelles - EpiDrive')
@section('meta_description', 'Gerez vos donnees personnelles conformement au RGPD.')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12" x-data="{ showDeleteModal: false }">

    <nav class="text-sm text-gray-500 mb-6">
        <a href="{{ route('account.index') }}" class="hover:text-emerald-700 transition">Mon compte</a>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 sm:p-8 mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                ...
                <p class="text-gray-700 font-medium">{{ $user->phone ?? 'Non renseigne' }}</p>
            </div>
            <div class="bg-gray-50 rounded-xl p-4">
                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Adresse</p>
                <p class="text-gray-700 font-medium">{{ $user->address ?? 'Non renseignee' }}</p>
            </div>
            <div class="bg-gray-50 rounded-xl p-4">
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                ...
                <p class="text-gray-700 font-medium">{{ $orders->count() }}</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 sm:p-8 mb-6">
        ...
        <form action="{{ route('account.export-data') }}" method="POST">
            @csrf
            <button type="submit"
